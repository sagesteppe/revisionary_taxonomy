...
window_search <- function(x){
  ...
  # determine which couplets are true pairs
  ...
  rm(couplets, equivalent, equi, true_couplets, aLL)
  ...
  # step 4 ###################################
  # find identical groups in the data set
  
  
  # step 5 ########################################
  # identify groups with shared individuals
  
  b <- map(multiples, data.frame) %>% 
  ...
  ident_grps$vals <- lapply(multiples, unlist)
  ...
  x <- x[str_detect(names(x), grps_keep, negate = F)]
  return(x)
