...
colnames(ident_grps)[1] <- "newname2"
...
vect <- sort(unique(c(unlist(id1$vals))))
func <- function(x){setdiff(vect, unlist(x))}
missing_vals <- func(longest$vals)
...
my_intersect <- function(x){
  ...
  res <- ifelse(res == missing_vals, 1, 0)
  res <- sum(res/length(res))
  return(res)
}

prop_rmn_cptr <- apply(id1, 1, my_intersect)

cbind(id1, prop_rmn_cptr)
