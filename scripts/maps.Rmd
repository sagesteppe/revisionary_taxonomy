...
rm(cnty_rds, counties_filt, counties_filt1, topo_base, roads_coarse, basemap, spp_cols,
   closest_sites, grps_most_dispersed, john_tigger, population_finder, reed_csv)
...
# create an id system for the list.
...
rm(couplets, equivalent, equ, true_couplets)

############################################
# part 3 subset original dataset with removed points,
...
#ident_grps$length <- lapply(multiples, length)
ident_grps$vals <- lapply(multiples, unlist)
ident_grps <- tibble(ident_grps)	%>%
  rowwise() %>% 
  mutate(members_L = length(vals))	

id1 <- filter(ident_grps, Group == 1) 

length(
  setdiff(unlist(id1$vals[14]), unlist(id1$vals[2]))
  )

setdiff(unlist(id1$vals[2]), unlist(id1$vals[14]))

alpha <- unlist(id1$vals[14])
beta <- unlist(id1$vals[2])
setdiff(beta, alpha)

crap <- Reduce(unique, list(id1$vals[1],id1$vals[2],id1$vals[3]))
